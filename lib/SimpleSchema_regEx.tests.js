...
describe('SimpleSchema', function () {
    it('regEx - issue 409', async function () {
        ...
        expect((await schema.newContext().validate({}))).to.deep.equal(true);
        expect((await schema.newContext().validate({foo: null}))).to.deep.equal(true);
        expect((await schema.newContext().validate({foo: ''}))).to.deep.equal(false);
    });

    it('no regEx errors for empty strings when `skipRegExCheckForEmptyStrings` field option is true',
        async function () {
            const schema = new SimpleSchema({
                foo: {
                    type: String,
            ...
            expect((await schema.newContext().validate({foo: ''}))).to.equal(true);

            // still fails when not empty string, though
            expect((await schema.newContext().validate({foo: 'bad'}))).to.equal(false);
        });

    it('Built-In RegEx and Messages', async function () {
        const schema = new SimpleSchema({
            email: {
                type: String,
    ...
    it('Optional regEx in subobject', async function () {
        const schema = new SimpleSchema({
            foo: {
                type: Object,
                optional: true,
        ...
        expect((await context.validate({}))).to.deep.equal(true);
    ...
    it('SimpleSchema.RegEx.idOfLength', function () {
        ...
        const expectThrows = (min, max) => expect(() => idOfLength(min, max)).to.throw(
