...
describe('SimpleSchema', function () {
    ...
    describe('nesting', function () {
        ...
        it('expects a field with SimpleSchema type to be an object', async function () {
        ...
        it('includes type validation errors from nested schemas', async function () {
            const schema = new SimpleSchema({
                foo: new SimpleSchema({
                    bar: String,
                }),
        ...
        it('includes allowed value validation errors from nested schemas', async function () {
            const schema = new SimpleSchema({
                foo: new SimpleSchema({
                    bar: {
                        type: String,
        ...
        it('includes validation errors from nested schemas when validating modifiers', async function () {
            const schema = new SimpleSchema({
                foo: new SimpleSchema({
                    bar: String,
                }),
        ...
        it('validates nested requiredness', async function () {
            const schema = new SimpleSchema({
                a: {
    ...
    it('Issue #123', async function () {
        ...
        expect(await context2.validate({
            $push: {
                profile: {},
            },
        }, {modifier: true})).to.equal(false);
    ...
    it('validate object with prototype', async function () {
        const schema = new SimpleSchema({
            foo: {type: SimpleSchema.Integer},
            bar: {type: SimpleSchema.oneOf(String, SimpleSchema.Integer)},
        });
        ...
        let validated = await context.validate(testObj);
        expect(validated).to.equal(true);
        expect(testObj instanceof CustomObject).to.equal(true);

        testObj.foo = 'not a number';
        validated = await context.validate(testObj);
        expect(validated).to.equal(false);
    });

    it('validate object with prototype within normal object', async function () {
        const schema = new SimpleSchema({
            customObject: Object,
        ...
        const context = schema.namedContext();
        expect(await context.validate(testObj)).to.equal(true);
        expect(testObj.customObject instanceof CustomObject).to.equal(true);

        testObj.customObject.foo = 'not a number';
        expect(await context.validate(testObj)).to.equal(false);
    });
    ...
    it('validating an object with a "length" property should not error', async function () {
        const schema = new SimpleSchema({
            length: {
                type: Number,
                optional: true,
    ...
    it('issue 360', async function () {
        ...
        const test1 = await expectErrorOfTypeLength(SimpleSchema.ErrorTypes.EXPECTED_TYPE, schema, {
            emails: [
                {
                    address: 12321,
                    verified: 'asdasd',
        ...
        const test2 = await expectErrorOfTypeLength(SimpleSchema.ErrorTypes.EXPECTED_TYPE, schema, {
            emails: [
                {
                    address: 12321,
                    verified: 'asdasd',
    ...
    it('ignore option', async function () {
        const schema = new SimpleSchema({
        ...
        await expectErrorOfTypeLength(SimpleSchema.ErrorTypes.KEY_NOT_IN_SCHEMA, schema, {
            bar: 'foo',
        });
    ...
    it('ClientError', async function () {
        ...
        try {
            await schema.validate({int: '5'});
        } catch (error) {
            verify(error);
        }

        try {
            await SimpleSchema.validate({int: '5'}, schema);
        } catch (error) {
            verify(error);
        }

        try {
            await SimpleSchema.validate({int: '5'}, {
                int: SimpleSchema.Integer,
                string: String,
            });
        } catch (error) {
        ...
        try {
            await schema.validator()({int: '5'});
        } catch (error) {
            verify(error);
        }

        await schema.validator({clean: true})({int: '5', string: 'test'});
    });
    ...
    it('getFormValidator', function () {
        ...
        const formValidator = schema.getFormValidator();
        const cleanFormValidator = schema.getFormValidator({clean: true});
        return Promise.all([
            formValidator({int: '5'})
                .then((errors) => {
            ...
            cleanFormValidator({int: '5', string: 'test'})
                .then((errors) => {
                    expect(errors).to.deep.equal([]);
                }),
        ]);
    ...
    it('validate takes an array', async function () {
        const schema = new SimpleSchema({
            int: SimpleSchema.Integer,
            string: String,
        });
        ...
        await expectValidationError(schema.validate([{int: 5, string: 'test'}, {int: '5'}]));

        await expectValidationError(SimpleSchema.validate([{int: 5, string: 'test'}, {int: '5'}], schema));

        await expectValidationError(SimpleSchema.validate([{int: 5, string: 'test'}, {int: '5'}], {
            int: SimpleSchema.Integer,
            string: String,
        }));

        await expectValidationError(schema.validator()([{int: 5, string: 'test'}, {int: '5'}]));
    });

    it('validationErrorTransform', async function () {
        const schema = new SimpleSchema({
            string: String,
        });
    ...
    it('SimpleSchema.addDocValidator', async function () {
        ...
        SimpleSchema.addDocValidator((obj) => {
            //Next line was throwing client console errors and isn't needed.
            //We just need the validation to fail so it is disabled.
        ...
        const context = schema.newContext();
        try {
            await context.validate(validatedObject);
        } catch (e) {
            //We dont need to do anything, this is to suppress console warning
        }

        const validationErrors = context.validationErrors();
        expect(validationErrors).to.deep.equal(errorArray);
    ...
    it('addDocValidator', async function () {
        const schema = new SimpleSchema({
            string: String,
        });
        ...
        schema.addDocValidator(async (obj) => {
            expect(obj).to.deep.equal(validatedObject);
            return errorArray;
        });
    ...
    it('$currentDate Date validation', async function () {
        const schema = new SimpleSchema({
            date: Date,
        });
        const context = schema.namedContext();
        ...
        expect((await context.validate(testModifer, {modifier: true}))).to.equal(true);
        ...
        await context.validate(testModifer, {modifier: true});
        expect((await context.validate(testModifer, {modifier: true}))).to.equal(true);
        ...
        expect((await context.validate(testModifer, {modifier: true}))).to.equal(false);
    });
    ...
    describe('SimpleSchema.Any', function () {
        ...
        describe('with modifiers', function () {
            ...
            describe('$unset', async function () {
                it('can be used for $unset modifiers', async function () {
                    (await expectErrorOfTypeLength(
                ...
                it('can be used for nested $unset modifiers', async function () {
                    const parentSchema = new SimpleSchema({parent: schema});
                    (await expectErrorOfTypeLength(
                        SimpleSchema.ErrorTypes.REQUIRED,
                        parentSchema,
                ...
                it('can be used for nested $unset modifiers with dot notation', async function () {
                    const parentSchema = new SimpleSchema({parent: schema});
                    (await expectErrorOfTypeLength(
                        SimpleSchema.ErrorTypes.REQUIRED,
                        parentSchema,
            ...
            describe('$addToSet', function () {
                it('can be used for $addToSet modifiers', async function () {
                    await expectValid(
                        schema,
                        {$addToSet: {testAny: 1}},
                        {modifier: true},
                ...
                it('can be used for nested $addToSet modifiers with dot notation', async function () {
                    const parentSchema = new SimpleSchema({parent: schema});
