  ...
  // Constructor function checks
    ...
    if (expectedType === Date) {
      ...
      if (_.isDate(def.min) && def.min.getTime() > keyValue.getTime()) {
        return {type: SimpleSchema.ErrorTypes.MIN_DATE, min: Utility.dateToDateString(def.min)};
      } else if (_.isDate(def.max) && def.max.getTime() < keyValue.getTime()) {
        return {type: SimpleSchema.ErrorTypes.MAX_DATE, max: Utility.dateToDateString(def.max)};
      }
    }
  }
}
