...
class SimpleSchema {
  ...
  async validate(obj, options = {}) {
    ...
    for(const oneObj of objects) {
      const validationContext = this.newContext();
      try {
        const isValid =  await validationContext.validate(oneObj, options);
      ...
      if (typeof SimpleSchema.validationErrorTransform === 'function') {
        ...
        console.log('gunna chuck an error');
        throw error;
      }
    };
  }
   ...
   */
  async validateAndReturnErrorsPromise(obj, options) {
    const validationContext = this.newContext();
    const isValid = await validationContext.validate(obj, options);
  ...
  validator(options = {}) {
    return async (obj) => {
      ...
      if (options.returnErrorsPromise) {
        return await this.validateAndReturnErrorsPromise(obj, optionsClone);
      }
      return await this.validate(obj, optionsClone);
    };
  }
