...
import {appendAffectedKey, getParentOfKey, isObjectWeShouldTraverse, looksLikeModifier} from './utility';
...
async function doValidation({
    ...
    let validationErrors = []; //matt woz ere

    // Validation function called for each affected key
    async function validate(val, affectedKey, affectedKeyGeneric, def, op, isInArrayItemObject, isInSubObject) {
        ...
        const validatorContext = {
            ...
            valueShouldBeChecked: (
                op !== '$unset' && op !== '$rename' &&
                ((val !== undefined && val !== null) || (affectedKeyGeneric.slice(
                    -2) === '.$' && val === null && !def.optional))
            ),
            ...(extendedCustomContext || {}),
        };
        ...
        async function validateField(typeDef) {
            ...
            const promises = fieldValidators.map(async (validator) => {
                let result;
                try {
                    result = await validator.call(finalValidatorContext);
                } catch (e) {
                    throw e;
                }
        ...
        const promises = def.type.map(async (typeDef) => await validateField(typeDef));
        let fieldValidationResults = [];
        try {
            fieldValidationResults = await Promise.all(promises);
        } catch (e) {
            throw (e);
        }

        const allFieldsValid = fieldValidationResults.includes(true);

        if (!allFieldsValid) {
            validationErrors = validationErrors.concat(fieldValidationErrors);
        }

        return fieldValidationErrors; //Needed for async?
    }
    ...
    async function checkObj({
        ...
        if (affectedKey) {
            ...
            if (shouldValidateKey) {
                try {
                    await validate(val, affectedKey, affectedKeyGeneric, def, operator, isInArrayItemObject,
                        isInSubObject);
                } catch (e) {
                    throw e;
                }
    ...
    async function checkModifier(mod) {
        // Loop through operators
        for (const op of Object.keys(mod)) {
            const opObj = mod[op];
            ...
            if (shouldCheck(op)) {
                ...
                if (isUpsert && (op === '$set' || op === '$setOnInsert')) {
                    ...
                    const oks = schema.objectKeys();
                    for (const schemaKey of oks) {
                ...
                for (let k of Object.keys(opObj)) {
                    let v = opObj[k];
    ...
    for (const func of docValidators) {
        let errors = null;
        try {
            errors = await func.call(docValidatorContext, obj);
        } catch (e) {
            throw e;
        }
