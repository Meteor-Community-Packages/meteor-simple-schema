...
SimpleSchema.prototype.clean = function (doc, options = {}) {
  ...
  let mDoc = new MongoObject(doc, this._blackboxKeys);

  // Clean loop
  if (options.filter || options.autoConvert || options.removeEmptyStrings || options.trimStrings) {
    let removedPositions = []; // For removing now-empty objects after

    mDoc.forEachNode(function() {
      let gKey = this.genericKey, p, def, val;
      ...
      if (gKey) {
        ...
        if (val !== void 0) {
          ...
          if (options.autoConvert && this.operator !== "$unset" && def) {
            let newVal = typeconvert(val, def.type);
    ...
    for (let removedPosition of removedPositions) {
      let lastBrace = removedPosition.lastIndexOf('[');
      if (lastBrace === -1) continue;
      let removedPositionParent = removedPosition.slice(0, lastBrace);
      let value = mDoc.getValueForPosition(removedPositionParent);
  ...
  options.getAutoValues && getAutoValues.call(this, mDoc, options.isModifier, options.extendAutoValueContext);

  // Ensure we don't have any operators set to an empty object
  // since MongoDB 2.6+ will throw errors.
  if (options.isModifier) {
    for (var op in doc) {
      if (doc.hasOwnProperty(op) && _.isEmpty(doc[op])) delete doc[op];
    }
  }
...
function typeconvert(value, type) {
  ...
  if (Array.isArray(value) || (_.isObject(value) && !(value instanceof Date))) return value;
...
function getAutoValues(mDoc, isModifier, extendedAutoValueContext) {
  ...
  _.each(this._autoValues, function(func, fieldName) {
