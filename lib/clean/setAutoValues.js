 ...
 * @param {Object} [extendedAutoValueContext] - Object that will be added to the context when calling each autoValue
 *     function
 * @returns {undefined}
 *
 ...
 */
async function setAutoValues(autoValueFunctions, mongoObject, isModifier, isUpsert, extendedAutoValueContext) {
    ...
    for (const sortedItem of sortedAutoValueFunctions) {
        ...
        for (const position of positions) {
            await avRunner.runForPosition(position);
        }
    }
}
