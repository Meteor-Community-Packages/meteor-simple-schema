...
MongoObject = function(objOrModifier, blackBoxKeys) {
  ...
  self.forEachNode = function(func, options) {
    ...
    _.each(self._affectedKeys, function(affectedKey, position) {
      ...
      func.call({
        ...
        isArrayItem: self._arrayItemPositions.indexOf(position) > -1,
  ...
  self.getValueForPosition = function(position) {
    ...
    for (var i = 0, ln = subkeys.length; i < ln; i++) {
      ...
      if (!_.isArray(current) && !isBasicObject(current) && i < ln - 1) return;
    }
   ...
   * Example: ['foo[bar][0]']
   */
  self.getPositionsInfoForGenericKey = function(key) {
    // Get the info
    var exactPositions = [];
  ...
  self.removeArrayItems = function () {
    // Traverse and pull out removed array items at this point
    function traverse(obj) {
      _.each(obj, function (val, indexOrProp) {
        // Move deeper into the object
 ...
 */
MongoObject.makeKeyGeneric = function makeKeyGeneric(key) {
