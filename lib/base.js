...
import humanize from './string-humanize.js';
  ...
  constructor(schemas, options) {
    ...
    _.each(this._schema, (definition, fieldName) => {
      if (definition instanceof SimpleSchemaGroup) {
        definition.definitions = definition.definitions.map((def) => {
...
SimpleSchemaGroup = class SimpleSchemaGroup {
  constructor(...definitions) {
    this.definitions = definitions.map((definition) => {
      if (Utility.isBasicObject(definition)) return definition;
...
function expandSchema(schema) {
  ...
  let addArrayFields = [];
  _.each(schema, function (definition, key) {
    if (!Utility.isBasicObject(definition) && !(definition instanceof SimpleSchemaGroup)) {
      if (_.isArray(definition)) {
        ...
        if (type instanceof RegExp) {
          ...
          addArrayFields.push({key: key, type});
        }
        schema[key] = {
          type: Array
        };
  ...
  for (let {key, type, regEx} of addArrayFields) {
    let itemKey = `${key}.$`;
    if (schema[itemKey]) {
...
function checkAndScrubDefinition(fieldName, definition, options) {
  ...
  let internalDefinition = _.extend({}, definition);
    ...
    if (!internalDefinition.hasOwnProperty('optional')) {
      if (internalDefinition.hasOwnProperty('required')) {
        if (typeof internalDefinition.required === 'function') {
          internalDefinition.optional = function () {
            return !internalDefinition.required.apply(this, arguments);
          };
        } else {
