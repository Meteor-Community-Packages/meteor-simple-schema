SimpleSchema.ValidationContext = class {
  ...
  setInvalidKeys(errors) {
    let previousInvalidKeys = _.pluck(this._invalidKeys, 'name');
    let newInvalidKeys = _.pluck(errors, 'name');

    this._invalidKeys = errors;

    // Mark all previous plus all new as changed
    let changedKeys = previousInvalidKeys.concat(newInvalidKeys);
  ...
  addInvalidKeys(errors) {
    ...
    this._markKeysChanged(newInvalidKeys);
  }
  ...
  keyErrorMessage(key, genericKey = MongoObject.makeKeyGeneric(key)) {
    ...
    return this._simpleSchema.messageForError(errorObj.type, errorObj.name, null, errorObj.value, errorObj.message);
  }
  ...
  validate(doc, {
    ...
    let invalidKeys = doValidation(doc, modifier, upsert, keys, this._simpleSchema, extendedCustomContext, ignore);
    ...
    if (keys) {
      ...
      for (let error of this._invalidKeys) {
        let wasValidated = _.any(keys, (key) => {
          return key === error.name || error.name.startsWith(key + '.');
        });
        if (!wasValidated) invalidKeys.push(error);
      }
    }

    this.setInvalidKeys(invalidKeys);
  ...
  isValid() {
    ...
    return this._invalidKeys.length === 0;
  }
