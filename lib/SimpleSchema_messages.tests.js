...
describe('SimpleSchema', function () {
    describe('messages', function () {
        ...
        it('maxNumber', async function () {
            ...
            const context = schema.newContext();
            await context.validate({foo: 2.5});
            expect(context.keyErrorMessage('foo')).to.equal('Foo cannot exceed 2');
        });

        it('minNumberExclusive', async function () {
            const schema = new SimpleSchema({
                foo: {
                    type: Number,
                    min: 2,
        ...
        it('minDate', async function () {
            ...
            const context = schema.newContext();
            await context.validate({foo: new Date(Date.UTC(2015, 10, 15, 0, 0, 0, 0))});
            expect(context.keyErrorMessage('foo')).to.equal('Foo must be on or after 2015-12-15');
        });

        it('maxDate', async function () {
            const schema = new SimpleSchema({
                foo: {
                    type: Date,
            ...
            const context = schema.newContext();
            await context.validate({foo: new Date(Date.UTC(2016, 1, 15, 0, 0, 0, 0))});
            expect(context.keyErrorMessage('foo')).to.equal('Foo cannot be after 2015-12-15');
        });

        it('badDate', async function () {
            const schema = new SimpleSchema({
                foo: {
                    type: Date,
                },
            ...
            const context = schema.newContext();
            await context.validate({foo: new Date('invalid')});
            expect(context.keyErrorMessage('foo')).to.equal('Foo is not a valid date');
        });

        it('minCount', async function () {
            const schema = new SimpleSchema({
                foo: {
                    type: Array,
                    minCount: 2,
        ...
        it('regEx built in', async function () {
            const schema = new SimpleSchema({
                foo: {
                    type: String,
        ...
        describe('keyNotInSchema', function () {
            ...
            it('$unset does not complain', async function () {
                const context = schema.newContext();
                await context.validate({
                    $unset: {
                        bar: '',
        ...
        it('should allow labels with apostrophes ("\'") in messages', async function () {
            const schema = new SimpleSchema({
                foo: {
                    type: String,
    ...
    describe('multipleSchema', function () {
        ...
        it('should keep message boxes separate between objects', async function () {
            const context = schema.newContext();
            await context.validate({});
