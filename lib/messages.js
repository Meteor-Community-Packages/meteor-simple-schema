...
SimpleSchema._globalMessages = {
  ...
  expectedType: "[label] must be of type [type]",
  regEx: [
  ...
  if (options.extendRegEx) {
    for (let prop in messages) {
      if (messages.hasOwnProperty(prop) &&
          prop.startsWith('regEx') &&
          _.isArray(messages[prop]) &&
          _.isArray(this._messages[prop])) {
        messages[prop] = mergeRegEx(this._messages[prop], messages[prop]);
      }
    }
  }

  _.extend(this._messages, messages);
  this._depsMessages.changed();
};
...
SimpleSchema.prototype.messageForError = function ({ type, name, value, message }) {
  ...
  let typePlusKey = `${type} ${name}`;
  let genericKey = MongoObject.makeKeyGeneric(name);
  let typePlusGenKey = `${type} ${genericKey}`;

  // reactively update when message templates are changed
  SimpleSchema._depsGlobalMessages.depend();
  this._depsMessages.depend();
  ...
  function findRegExError(message) {
    ...
    if (regExpMatch) {
      msgObj = _.find(message, (o) => o.exp && o.exp.toString() === regExpMatch);
    }
  ...
  message = message ||                                      // Passed in message
                ...
                this._messages[typePlusKey] ||                  // (1) Use schema-specific message for specific key
                this._messages[typePlusGenKey] ||               // (2) Use schema-specific message for generic key
                this._messages[type];                           // (3) Use schema-specific message for type
  ...
  message = message.replace("[label]", this.label(name));
  ...
  // [value]
  // Use ''+value instead of value.toString() in case value is null or undefined
  ...
  // [type]
  if (def.type === SimpleSchema.Integer) {
    message = message.replace('[type]', 'Integer');
...
function mergeRegEx(currentRegEx, newRegEx) {
  _.each(newRegEx, function (obj) {
    let idx = _.findIndex(currentRegEx, function (currentObj) {
