...
By default, every time you call `messages`, any keys that begin with "regEx" will completely replace any existing message array for that key. If you want to intelligently extend and replace items in the existing array instead, pass the `extendRegEx: true` option to `messages`.
