# aldeed:simple-schema (community-maintained Meteor package)

This is the community-maintained version of SimplSchema, that provides compatibility for Meteor 3.x and async/await.
If you don't intend to use this within Meteor, then please [use the npm package `simpl-schema`](https://github.com/longshotlabs/simpl-schema).
...
- Used by the [Collection2](https://github.com/Meteor-Community-Packages/meteor-collection2) and [AutoForm](https://github.com/Meteor-Community-Packages/meteor-autoform) Meteor packages (both community-maintained).
...
Since version 3.x of the NPM package there is a breaking change that dropped all Meteor support. 
This is why we did contact Aldeed to allow us to move it back to `aldeed:simple-schema` under the realm of the Meteor Community Packages.
...
We removed the `SimpleSchema.regEx` from this library, due to security concerns (like redos vulnerability).
One exception is the `idOfLength` regEx, which contains no runaway pattern and is also internally used to match
Meteor's mongo ids.

Furthermore, we kept the signatures, so you can define your custom RegExp for each entry via `SimpleSchema.defineRegex`.
The following example registers a safer version of the Email regex using [`email-regex-safe`](https://www.npmjs.com/package/email-regex-safe):

```js
import emailRegxSafe from 'email-regex-safe'

SimpleSchema.defineRegex(
  SimpleSchema.RegEx.EmailWithTLD,
  emailRegxSafe({ strict: true })
)
```

Also note, that they are functions now. The reason behind this is, that messages
